...
class MainWindow(QMainWindow):
    ...
    def run_stage(self, stage_name, stage_function):
        ...
        try:
            ...
            # Log the FULL traceback (file + line number) to the debug log file
            # so the exact crash site can be identified — str(e) alone (used in
            # the popup below) only shows the bare exception text, which for
            # OpenCV errors gives no clue which line/file actually crashed.
            import traceback
            from utils.logger import error as log_error
            log_error(f"[{stage_name}] Unhandled exception:\n{traceback.format_exc()}")
