...
def epic_raid_stage():
    ...
    while should_continue(tickets_used):
        ...
        if check_network_error(screenshot=screenshot):
            continue
        if check_image_present("assets/icons/create_room_epic_raid.png", screenshot=screenshot):
            debug("[Stage] In menu creating room.")

            find_and_click_image("assets/icons/create_room_epic_raid.png", screenshot=screenshot)
            ...
            if find_and_click_image("assets/icons/start_quest_epic_raid.png", screenshot=screenshot):
                ...
                screenshot = get_bbs_screenshot()
                if check_image_present("assets/icons/start_quest.png", screenshot=screenshot):
                    find_and_click_image("assets/icons/start_quest.png", screenshot=screenshot)

        elif check_image_present("assets/icons/public_small.png", screenshot=screenshot):
            debug("[Stage] In menu setting room to public.")
            find_and_click_image("assets/icons/public_small.png", screenshot=screenshot)
            screenshot = get_bbs_screenshot()
                ...
                while in_menu:
                    ...
                    if (not check_image_present("assets/icons/epic_raid_locked.png", screenshot=screenshot)):
                        ...
                        if (find_and_click_image("assets/icons/start_quest_epic_raid.png", screenshot=screenshot)):
                            ...
                            screenshot = get_bbs_screenshot()
                            if check_image_present("assets/icons/members_not_ready_epic_raid.png", screenshot=screenshot):
                                coop_start_tries += 1
        ...
        time.sleep(2)  # was 5 — tightened outer poll interval
...
def should_continue(tickets_used: int) -> bool:
    ...
    boost_amount = 5 if settings["auto_set_boost_to_max"] else 0
