...
def brave_battles():
    ...
    battles_completed = 0
    not_on_pvp_counter = 0  # grace period in case the screen is still loading
    ...
    while not stop_controller.should_stop():
        ...
        if check_image_present("assets/icons/brave_battle_check.png", screenshot=screenshot):
            ...
            if check_image_present("assets/icons/brave_battles_battle.png", screenshot=screenshot):
                ...
                if find_and_click_image("assets/icons/brave_battles_battle.png", screenshot=screenshot):
                    time.sleep(0.3)  # was 1 — tightened
            ...
            not_on_pvp_counter += 1
            debug(f"[Brave Battles] Not on PvP screen yet ({not_on_pvp_counter}/10) — may still be loading.")
            if not_on_pvp_counter >= 10:
                return "Not on PvP screen. Please navigate to Brave Battles first."
            time.sleep(0.15)  # tightened for 8x speed
            continue
        ...
        time.sleep(1)  # was 2 — tightened outer poll interval
